...
export const useDataStore = defineStore("data", {
	...
	getters: {
		...
		checkEmptyField() {
			...
			for (let i = 0; i < this.fields.length; i++) {
				...
				if (field.typeId == 6) {
					// contact
					let description = this.fieldsData.descriptions[field.propsId];
					if (field.deleted == 1) {
						// pass
					} else {
						if (description.title == "" || description.value == "") {
	...
	actions: {
		construct() {
			this.apiServer = this.server + "api/";
			this.logoServer = this.server + "uploads/restorants/";
			this.imageServer = this.server + "uploads/card_images/";
		},
		selectTab(tab) {
		...
		async getData() {
			...
			// get faqs
			// url = this.apiServer + "get-shop-faqs/" + this.shopId;
			// try {
			// 	let data = await axios.get(url);
			// 	data = data.data;
			...
			try {
				...
				data.forEach((field, index) => {
					tempFields.push({
						...
						inp: this.refData.fieldsTypeMap[parseInt(field.type)].inp,
						out: this.refData.fieldsTypeMap[parseInt(field.type)].out,
		...
		async postData() {
			...
			// put faqs
			// url = this.apiServer + "update-shop-faqs/" + this.shopId;
			// try {
			// 	let resp = await axios.put(url, this.formattedFaqs, {
			// 		headers: {
			// 			"Content-type": "application/json",
			// 		},
			// 	});
		...
		async getAllImages() {
			let url = this.apiServer + "get-all-uploaded-shop-images/" + this.shopId;
			try {
				let data = await axios.get(url);
				data = data.data;
		...
		setImageOnField() {
			this.fieldsData.images[this.refData.currentImagePropsId].value =
				this.fieldsData.uploadedImages[this.refData.currentSelectedImage].image;
		},
		...
		addImage() {
			...
			this.fieldsData.images.push({ propsId: mxId, title: "", value: "" });
			this.fields.push({
				typeId: 2,
				propsId: mxId,
				inp: InpImageField,
		...
		addVideo() {
			...
			this.fieldsData.videos.push({ propsId: mxId, title: "", value: "" });
			this.fields.push({
				typeId: 3,
				propsId: mxId,
				inp: InpVideoField,
		...
		addFaq() {
			...
			this.fieldsData.faqs.push({ propsId: mxId, question: "", answer: "" });
			this.fields.push({
				typeId: 7,
				propsId: mxId,
				inp: InpFaqField,
